...
export type AdminUpdateProgramFeeArgs = {
  newProgramFee: BN;
};

export class AdminUpdateProgramFee {
  ...
  public async adminUpdateProgramFee({
    newProgramFee,
  }: AdminUpdateProgramFeeArgs): Promise<TransactionInstruction> {
    const configPDA = await this.account.config.pda();
    const configData = await this.account.config.data(configPDA.address);
    return this.cronos.instruction.adminUpdateProgramFee(newProgramFee, {
      accounts: {

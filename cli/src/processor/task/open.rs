...
pub fn open(client: &Arc<Client>, ix: Instruction, schedule: String) -> Result<(), CliError> {
    // Fetch daemon data.
    ...
    let task_ix = cronos_sdk::instruction::task_open(task_pda, daemon_addr, owner, vec![ix], schedule);

...
pub fn create(client: &Arc<Client>) -> Result<(), CliError> {
    ...
    let yogi_pda = cronos_sdk::scheduler::state::Yogi::pda(owner);
    let fee_pda = cronos_sdk::scheduler::state::Fee::pda(yogi_pda.0);
    let ix = cronos_sdk::scheduler::instruction::yogi_new(fee_pda.0, owner, owner, yogi_pda.0);
    sign_and_submit(client, &[ix], &[client.payer()]);
    get(client, &yogi_pda.0)
}
...
pub fn get(client: &Arc<Client>, address: &Pubkey) -> Result<(), CliError> {
    ...
    let yogi_data = cronos_sdk::scheduler::state::Yogi::try_from(data)

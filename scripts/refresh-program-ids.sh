...
program_id_delegate=$(solana address -k target/deploy/cronos_delegate-keypair.json)
...
sed -i '' -e 's/^declare_id!(".*");/declare_id!("'${program_id_delegate}'");/g' programs/delegate/src/id.rs
...
sed -i '' -e 's/^delegate = ".*"/delegate = "'$program_id_delegate'"/g' Anchor.toml
